...
export default {
	...
	gas: "200000" }, memo})
		...
		async sendMsgMarkSpace({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const txClient=await initTxClient(rootGetters)
				const msg = await txClient.msgMarkSpace(value)
				const result = await txClient.signAndBroadcast([msg], {fee: { amount: fee, 
	gas: "200000" }, memo})
				return result
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgMarkSpace:Init Could not initialize signing client. Wallet is required.')
				}else{
					throw new Error('TxClient:MsgMarkSpace:Send Could not broadcast Tx: '+ e.message)
				}
			}
		},
	...
	gas: "200000" }, memo})
		...
		async MsgMarkSpace({ rootGetters }, { value }) {
			try {
				const txClient=await initTxClient(rootGetters)
				const msg = await txClient.msgMarkSpace(value)
				return msg
				...
				if (e == MissingWalletError) {
					...
					throw new Error('TxClient:MsgMarkSpace:Create Could not create message: ' + e.message)
				}
			}
		},
